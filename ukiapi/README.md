...
Core library for the UkiApi framework. Provides routing, extractors, and utilities for building FastAPI-inspired APIs in Rust.
...
- `UkiApi`: The main entry point to build your `axum::Router`.
- `routes!` macro: A convenient way to initialize your API with multiple routes.

## 🛠️ Usage Example

```rust
use ukiapi::{get, routes, APIRouter, AppState};
