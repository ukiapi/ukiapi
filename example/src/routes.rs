...
pub async fn create_item(
    ...
    let db_item = ItemDb {
        id: next_id,
        // ⚡ Bolt: Move fields directly from owned extractor to avoid body.name.clone()
        name: body.name,
        price: body.price,
    ...
    // ⚡ Bolt: Optimize memory by avoiding full clone of db_item
    // Only clone the name string needed for the response instead of the entire struct (including internal_secret)
    let response = ItemResponse {
        id: db_item.id,
