...
## 2024-07-22 - Missing Test for Streaming Error Propagation
**Issue:** `StreamingResponse` error propagation during stream consumption lacked unit test coverage.
**Learning:** In axum, stream errors generated from `Body::from_stream` are deferred until the stream is actively polled/collected (e.g., via `axum::body::to_bytes`), rather than failing eagerly during the `into_response()` conversion. Tests must assert on the result of `to_bytes` to verify deferred streaming errors.
**Prevention:** Always explicitly test stream body consumption using `axum::body::to_bytes` when verifying error propagation in streaming responses, rather than merely checking the initial HTTP status code.
