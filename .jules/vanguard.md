...
## 2024-08-03 - Test Missing Body on ValidatedJson Extractor
**Issue:** `ValidatedJson` extractor returned 422 for missing bodies, but this edge case lacked an explicit test with an empty body via `TestClient`.
**Learning:** Empty request payloads sent to JSON extractors correctly trigger an HTTP 422 Unprocessable Entity due to standard deserialization failures.
**Prevention:** Ensure all custom extractors explicitly test missing or completely empty request payloads, in addition to malformed/invalid payloads.
